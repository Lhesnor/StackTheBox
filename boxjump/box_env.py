...
class BoxAgent:
    def __init__(
        self, body: Box2D.b2Body, h: float = 0.666, v: float = 1, s: float = 1
    ):
        self.body = body
        self.yvels = []
        self.n = 10
...
class BoxJumpEnvironment(ParallelEnv):
    ...
    def __init__(
        ...
        max_timestep=400,
        fixed_rotation=True,
        reward_mode: str = "highest",
        include_time: bool = False,
        include_highest: bool = False,
        penalty_fall: float = 20,
        physics_steps_per_action=6,
        physics_timestep_multiplier=2.0,
        include_num_boxes: bool = True,
        termination_max_height: Optional[float] = None,
        termination_reward_coef: float = 100.0,
        termination_on_fall: bool = True,
        penalty_fall_termination: float = -100.0,
    ...
    def _load_emotion_images(self):
        ...
        for emotion, filename in emotion_files.items():
            ...
            if image is None:
                print(
                    f"Warning: Could not load emotion image '{filename}' from packaged assets"
                )
            ...
            scaled_image = pygame.transform.smoothscale(
                image, (int(self.box_width * SCALE), int(self.box_height * SCALE))
            )
    ...
    def get_all_obs(self):
        ...
        for i in range(self.num_boxes):
            ...
            x = xs[i]
            y = ys[i]
            relevant = np.logical_and(
                xs - w < x + w - threshold, xs + w > x - w + threshold
            )
            ...
            relevant = np.logical_and(
                ys - h < y + h - threshold, ys + h > y - h + threshold
            )
            if np.sum(relevant) == 1:
                left.append(1)
                right.append(1)
        ...
        for i in range(self.num_boxes):
            ...
            height_above_floor = (
                FLOOR_Y - b.body.position.y - self.box_height / 2
            ) / self.box_height
            ...
            ob = [
                ...
                b.body.linearVelocity.x
                / FPS
                * 20,  # constants chosen to scale approxmiately to [-1, 1]
        ...
        if self.include_time:
            state.append(
                np.array([self.timestep / self.max_timestep])
            )  # only include once
        if self.include_num_boxes:
    ...
    def _single_physics_step(self, actions):
        ...
        for idx, i in enumerate(self.agents):
            ...
            if action == 0:
                ...
                if not (
                    (mul > 0 and body.linearVelocity.x > MAX_HOR_SPD)
                    or (mul < 0 and body.linearVelocity.x < -MAX_HOR_SPD)
                ):
                    body.ApplyForceToCenter(
                        (mul * 10 / self.physics_timestep_multiplier, 0), True
                    )
            elif action == 3:
                if self.boxes[idx].can_jump():
                    body.ApplyForceToCenter(
                        (0, -250 / self.physics_timestep_multiplier), True
                    )

            # Calculate height for reward calculations
            # 0 = box sitting on the floor, 1 = directly on top of another box, ...
            height_above_floor = (
                FLOOR_Y - self.boxes[idx].body.position.y - self.box_height / 2
            ) / self.box_height
        ...
        for ind, i in enumerate(self.agents):
            height_above_floor = (
                FLOOR_Y - self.boxes[ind].body.position.y - self.box_height / 2
            ) / self.box_height
            ...
            if self.reward_mode == "highest" or self.reward_mode == "highest_stable":
                ...
                for box_idx, box in enumerate(self.boxes):
                    if box.can_jump():
                        box_height = (
                            FLOOR_Y - box.body.position.y - self.box_height / 2
                        ) / self.box_height
                ...
                for box_idx, box in enumerate(self.boxes):
                    if box.can_jump():
                        ...
                        max_stable_height = max(max_stable_height, box_height)
                exp_reward = (
                    (max_stable_height**2) / self.num_boxes / self.max_timestep * 10
                )
                rewards[i] = min(exp_reward, 1000.0) / self.physics_steps_per_action
            elif self.reward_mode == "dense_height_cube":
                exp_reward = (
                    (max_height_above_floor**3)
                    / self.num_boxes
                    / self.max_timestep
                    * 10
                )
                ...
                for box_idx, box in enumerate(self.boxes):
                    if box.can_jump():
                        ...
                        max_stable_height = max(max_stable_height, box_height)
                exp_reward = (
                    (max_stable_height**3) / self.num_boxes / self.max_timestep * 10
                )
                rewards[i] = min(exp_reward, 1000.0) / self.physics_steps_per_action
        ...
        if (self.termination_max_height is not None) and (
            new_best >= self.termination_max_height
        ):
            # Multiply current step rewards by termination_reward_coef and mark done
            rewards = {
                agent: rewards.get(agent, 0.0) * self.termination_reward_coef
                for agent in self.agents
            }
            done = True
    ...
    def _get_box_emotion(self, box_idx):
        ...
        is_highest = (
            abs(height_above_floor - max_height) < 0.1 and height_above_floor > 0.1
        )
        ...
        if (
            is_highest and height_above_floor > 0.9
        ):  # Only super happy if significantly high
            return "super_happy"
    ...
    def render(self, save_path=None):
        ...
        square_image = pygame.Surface(
            (self.box_width * SCALE, self.box_height * SCALE), pygame.SRCALPHA
        )
        ...
        pygame.draw.rect(
            self.screen,
            (0, 0, 0),
            (0, FLOOR_Y * SCALE, W * SCALE, (H - FLOOR_Y) * SCALE),
        )
